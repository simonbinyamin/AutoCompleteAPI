...
public class DockerController : ControllerBase
{
    ...
    {      
        if(key != secret.KEY) {
            ...
            using (var _client = new HttpClient())
            {
                ...
                var result = await _client.PatchAsync(secret.DOCKER_LIST+".json?auth=" + secret.API_KEY, content);
    ...
    public async Task<string> Delete(string index, string key)
    {
        if(key != secret.KEY) {
            ...
            using (var _client = new HttpClient())
            {
                var result = await _client.DeleteAsync(secret.DOCKER_LIST +"/"+ index + ".json?auth=" + secret.API_KEY);
    ...
    public async Task<ContentResult> List()
    {
        ...
        HttpResponseMessage response = await client.GetAsync(secret.DOCKER + ".json");
